...
def main():
    ...
    st.sidebar.markdown(" Web App para segmentar, através da convolução de diversos tipos de kernel, a região da placa em um carro")
    st.sidebar.markdown(" Ressalto que este projeto tem a finalidade de mostrar o que é possível realizar, utilizando apenas python e a biblioteca Open-CV para o processamento de imagens. Além de realizar a prova de conceito(POC) do projeto em questão")
    st.sidebar.markdown(" Utilizando Deep Learning, os resultados com certeza seriam melhores, logo é possível que em algumas fotos este pré-processamento de imagens não seja preciso...")
    st.sidebar.markdown(" Como premissa, considero que as imagens dos Automovéis são de boa resolução e com uma distância, relativa ao carro, curta.")

    # menu com oções de páginas
    opcoes_menu = ["Segmentar", "Sobre"]
    ...
    if escolha == "Segmentador de Placas":
        ...
        st.markdown("""
                            Projeto da Masterclass Introdução à Visão Computacional, do Carlos Melo (Sigmoidal).
        ...
        st.markdown(" Agora vamos aplicar um transformação morfológica, denominada Black Hat Transform(para saber mais [acesse](https://www.geeksforgeeks.org/top-hat-and-black-hat-transform-using-python-opencv/)).")
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (40, 13))#tamanho do kernel é proporcional ao tamanho da placa que queremos encontrar na imagem
        ...
        st.markdown(" A fim de ressaltar a placa, na imagem, aplicaremos um Blur através de uma função Gaussiana. Então fazemos uma operação de Close para filtrar ruídos e logo transformamos a imagem em binária:")
        ...
        st.markdown(" Realizamos uma operação de opening na imagem, para retirar ruídos brancos no fundo preto(para entender [acesse](https://docs.opencv.org/master/d9/d61/tutorial_py_morphological_ops.html))")
        thres = cv2.erode(thres, None, iterations=2)  # 2
        thres = cv2.dilate(thres, None, iterations=1) # 1
        st.image(thres)


        st.markdown("Por fim, com a função cv2.findContours, vamos segmentar a região da placa no carro e imprimir a imagem respectiva: ")
